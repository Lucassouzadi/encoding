...
* **file**
  ...
  * Descrição: Caminho do arquico que será codificado (caso a ação seja `encode` ou `encodeAndDecode`) ou decodificado (caso a ação seja `decode`).
...
* **arg**
  ...
  - Descrição: Parâmetro da codificação.
    ...
    * Para qualquer outra codificação, este parâmetro é desconsiderado
